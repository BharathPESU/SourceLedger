...
export const SignInView: React.FC = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (data.user && !data.user.email_confirmed_at && data.user.app_metadata?.provider === 'email') {
        setUnverifiedEmail(email.trim());
        setAuthMode('verify_email');
      }
    } catch (err: any) {
    ...
    >
      ...
      <form onSubmit={handleSignIn} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-[#191715] flex items-center justify-between">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-[11px] font-bold text-[#E8622C] hover:underline cursor-pointer"
            >
              Forgot password?
            </button>
          </div>
